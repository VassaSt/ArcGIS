  ...
  <script>
    ...
    window.addEventListener("message", async function (e) {
      ...
      if (JSON.stringify(property) != JSON.stringify(newProperty)) {
        ...
        API_URL = property.client_data.URL;
        ARCGIS_URL = property.client_data.arcgis_URL;
        client_id = property.client_data.client_ID;
        client_secret = property.client_data.client_secret;
        token_url = TokenURL + "?client_id=" + client_id + "&grant_type=client_credentials&client_secret=" + client_secret;
        hideLayers(property.data_list);
        get_token().then(() => {
          handleData(property);
        });
      }
      ...
      fetch(API_URL, {
        ...
        if (!data.hasOwnProperty("errorMessage")) {
          if (dataType == "kml") {
            ...
            // console.log("result kml:", result)
            showGeojson(result.data, result.centerPoint, itemID, dataType, dataItem)
          } else {
            let geojsonData = data;
            // console.log("result geojsonData :", geojsonData);
            let center = turf.center(geojsonData);
            showGeojson(geojsonData, center, itemID, dataType, dataItem)
          }
        } else {
          //Show error
    ...
    function handleData(files) {
      ...
      dataList.forEach(dataItem => {
        ...
        if (dataItem.layer_name && dataItem.symbol_ID) {
          ...
          // console.log(found);
          if (!found) {
            dataStore.push({ itemId: itemID, layerId: "", arr: [], centerPoint: [], })
          ...
          function moveCamera() {
            ...
            if ((file.type == "FeatureCollection") && (file.features[0].geometry.name === "markers")) {
              console.log(turf.bbox(file))
              reearth.camera.flyTo({
                lat: file.features[0].geometry.coordinates[1],
                lng: file.features[0].geometry.coordinates[0],
                height: 50000
              }, {
                duration: 2
              });
            } else if ((file.type == "FeatureCollection") && (file.features[0].geometry.type))  {
              let center = turf.center(file);
              reearth.camera.flyTo({
    ...
    let geojsonLayerId
    function showGeojson(data, center, itemID, dataType, dataItem) {
      ...
      if ((data.type == "FeatureCollection")) {
        ...
        switch (dataType) {
          ...
          case "point":
            ...
            console.log(points, multiPoints);
            ...
            turf.featureEach(multiPoints, function (currentFeature, featureIndex) {
              ...
              multipoints.forEach((el) => {
                ...
                // console.log("coords MultiPoint", circle);
            ...
            turf.featureEach(points, function (currentFeature, featureIndex) {
              ...
              circle = turf.circle(center, radius, options);
              circle.properties["fill"] = pointFill;
              // console.log("coords point", center);
          ...
          case "polygon":
            ...
            console.log("geoJson polygons", geoJson);
    ...
    function overridePolygonProperties(polygons, layerId, polygonFill, polygonProp) {
      ...
      // console.log("Polygons geoJson :", geoJson);
    ...
    function handleKMLFile(data) {
      ...
      // console.log("handleKMLFile(data) blob: ", blob)

      let result = URL.createObjectURL(blob);
      // console.log("handleKMLFile(data) result: ", result)
